...
use crate::_comde;
use crate::_comde::{Archive, ArchiveOpener, CompressStats, DecompressStats, ViewEntry, Viewer};
use crate::_error::DecompError;
use crate::_error::Error::{CompressError, DecompressError, IOError};
use crate::_error::IsIOError::{OnError, StdIoError};
use crate::_utils::calculate_size;
...
impl _comde::Compress for SevenzComde {
...
impl _comde::ComdeConfig for SevenzConfigs {
...
impl _comde::ComdeConfig for SevenzDeConfig {
...
impl _comde::Decompress for SevenzComde {
...
impl ArchiveOpener for SevenzComde {
    fn open_view<P>(path: P) -> crate::Result<Archive>
    where
        P: AsRef<Path>,
    {
        Ok(Archive::create(path.as_ref().to_path_buf(), &SevenzComde)?)
    }
}

impl Viewer for SevenzComde {
    fn get_entries(&self, path: &PathBuf) -> crate::Result<Vec<ViewEntry>> {
        todo!()
    }

    fn add_entry_to(&mut self, path_from: &PathBuf, to: &PathBuf) -> crate::Result<CompressStats> {
        todo!()
    }

    fn remove_entry_at(&mut self, path_at: &PathBuf) -> crate::Result<()> {
        todo!()
    }

    fn decompress_file(&self, path_at: &PathBuf, to: &PathBuf) -> crate::Result<DecompressStats> {
        todo!()
    }
}
