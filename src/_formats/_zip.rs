...
use crate::_comde;
use crate::_comde::{CompressStats, DecompressStats};
use crate::_error::DecompError::{PasswdIncorrect, PasswdNeeded};
...
impl _comde::Compress for ZipComde {
...
impl _comde::ComdeConfig for ZipConfig {
    fn new() -> Self {
        ZipConfig {
...
impl _comde::ComdeConfig for ZipDeConfig {
    fn new() -> Self {
...
impl _comde::Decompress for ZipComde {
        ...
        for i in 0..archive.len() {
            ...
            let is_encrypted = {
                let file_info = archive.by_index_raw(i).map_err(|e| {
                    DecompressError(crate::_error::DecompError::DecompressErr(e.to_string()))
                })?;
                file_info.encrypted()
            };
                    ...
                    match archive.by_index_decrypt(i, password.as_bytes()) {
                        ...
                        Err(e) => {
                            ...
                            return if e.to_string().contains("Invalid password") {
                                ...
                                Err(DecompressError(crate::_error::DecompError::DecompressErr(
                                    e.to_string(),
                                )))
                            };
                        }
